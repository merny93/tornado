...
def mass_calculator(sup_path, bins, plot = True):
    '''
...
if __name__ == '__main__':
    ...
    for i in range(len(angles)):
        res = mass_calculator(path.join('../data', 'tungsten/Angles/{}/'.format(angles[i])), [bins[i][0], bins[i][1]])
        peaks.append(res[0][0])
        uncertainty.append(res[1][0])
    np.savez(path.join('../data', 'tungsten/Angles/peaks.npz'), peaks = peaks, uncertainty = uncertainty)