...
if __name__ == '__main__':
    ...
    for i in range(len(peaks_data['angles'])):
        ...
        temp_mass = mass_electron(ufloat(peaks_data['peaks'][i], peaks_data['uncertainty'][i]), a, b, ufloat(peaks_data['angles'][i], 0.1) )
    ...
    plt.fill_between(np.linspace(0,359,360), np.ones(360)*avg-std,np.ones(360)*avg+std, color = 'orange', alpha = 0.5)
    plt.ylim(0.49, 0.54)
    plt.xlim(0,6)
